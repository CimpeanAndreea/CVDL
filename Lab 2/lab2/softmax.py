...
class SoftmaxClassifier:
    ...
    def initialize(self):
        ...
        self.W = np.random.randn(self.input_shape - 1, self.num_classes) * 0.001  # the image comes with 1 more column for the bias(1)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        dot_product = X.dot(self.W)
        scores = softmax(dot_product)
        return scores
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        for iteration in range(0, steps):
            ...
            batches_start_index = np.arange(0, len(X_train), bs)

            for start in batches_start_index:
                end = start + bs
                ...
                self.W -= lr * dW
                # append the training loss, accuracy on the training set and accuracy on the test set to the history
                # dict
                history.append(loss)
    ...
    def get_weights(self, img_shape):
        ...
        W_without_bias = self.W[:-1]
        W = np.reshape(W_without_bias, (*img_shape, self.num_classes))
        return W
